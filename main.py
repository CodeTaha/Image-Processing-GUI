...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.Worker1_Opencv = None
        self.value_total_object3 = None
        self.value_curr_object3 = None
        self.value_total_object2 = None
        self.value_curr_object2 = None
        self.value_total_object1 = None
        self.value_curr_object1 = None
        self.CopyImage = None
        self.hsvThree_upper = None
        self.hsvThree_lower = None
        self.hsvTwo_upper = None
        self.hsvTwo_lower = None
        self.hsvOne_upper = None
        self.hsvOne_lower = None
        self.DateTime = None
        self.RanColor3 = None
        self.RanColor2 = None
        self.RanColor1 = None
        ...
        self.setMinimumSize(640, 480)  # Minimum boyutları belirleyin
        self.setMaximumSize(1920, 1080)  # Maksimum boyutları belirleyin
    ...
    def GetObject_one(self):
        try:
            self.hsvOne_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            self.hsvOne_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)
    ...
    def GetObject_two(self):
        try:
            self.hsvTwo_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            self.hsvTwo_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)
    ...
    def GetObject_three(self):
        try:
            ...
            self.hsvThree_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            self.hsvThree_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)
    ...
    def opencv_emit(self, Image):
        ...
        if self.ckb_disp1.isChecked() and self.btn_setObject1.isChecked() == False and self.track_obj1.isChecked() == False:
            hsv_object_one_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            hsv_object_one_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)

            hsv_to_ObjectOne = cv2.cvtColor(src=self.CopyImage, code=cv2.COLOR_BGR2HSV)
            hsv_target_ObjectOne = cv2.inRange(src=hsv_to_ObjectOne, lowerb=hsv_object_one_lower,
                                               upperb=hsv_object_one_upper)
        ...
        if self.ckb_disp2.isChecked() and self.btn_setObject2.isChecked() == False:
            hsv_object_two_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            hsv_object_two_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)

            hsv_to_ObjectTwo = cv2.cvtColor(src=self.CopyImage, code=cv2.COLOR_BGR2HSV)
            hsv_target_ObjectTwo = cv2.inRange(src=hsv_to_ObjectTwo, lowerb=hsv_object_two_lower,
                                               upperb=hsv_object_two_upper)
        ...
        if self.ckb_disp3.isChecked() and self.btn_setObject3.isChecked() == False:
            hsv_object_three_lower = np.array(
                [self.Slider_H_min.value(), self.Slider_S_min.value(), self.Slider_V_min.value()], dtype=np.uint8)
            hsv_object_three_upper = np.array(
                [self.Slider_H_max.value(), self.Slider_S_max.value(), self.Slider_V_max.value()], dtype=np.uint8)

            hsv_to_ObjectThree = cv2.cvtColor(src=self.CopyImage, code=cv2.COLOR_BGR2HSV)
            hsv_target_ObjectThree = cv2.inRange(src=hsv_to_ObjectThree, lowerb=hsv_object_three_lower,
                                                 upperb=hsv_object_three_upper)
    ...
    def StopWebcam(self):
        self.textEdit.append(
            f"{self.DateTime.toString('d MMMM yy hh:mm:ss')}: Stop Webcam ({self.camlist.currentText()})")
    ...
    def Motor_state(self):
        if self.flag_motor:
            self.Motor_status.setPixmap(QPixmap('Icons/MotorON.png'))
        else:
            self.Motor_status.setPixmap(QPixmap('Icons/MotorOFF.png'))
    ...
    def Ready_lamp(self):
        if self.Status_lamp[0]:
            self.Qlabel_greenlight.setStyleSheet("background-color: rgb(85, 255, 0); border-radius:30px")
        else:
            self.Qlabel_greenlight.setStyleSheet("background-color: rgb(184, 230, 191); border-radius:30px")
    ...
    def Running_lamp(self):
        if self.Status_lamp[1]:
            self.Qlabel_yellowlight.setStyleSheet("background-color: rgb(255, 195, 0); border-radius:30px")
        else:
            self.Qlabel_yellowlight.setStyleSheet("background-color: rgb(242, 214, 117); border-radius:30px")
        self.Status_lamp[1] = not self.Status_lamp[1]
        # IO.output(IO_OUTPUT[1], self.Status_lamp[1])

    def Alarm_lamp(self):
        if self.Status_lamp[2]:
            self.Qlabel_yellowlight.setStyleSheet("background-color: rgb(255, 0, 0); border-radius:30px")
        else:
            self.Qlabel_yellowlight.setStyleSheet("background-color:  rgb(255, 171, 175); border-radius:30px")
